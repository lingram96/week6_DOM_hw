...
const handleFormSubmit = function(event){
    ...
    const newListItem4 = document.createElement ('h3');
    const newListItem5 = document.createElement ('h3');
    newListItem1.textContent = `${event.target.title.value}`;
    newListItem2.textContent = `${event.target.director.value}`;
    newListItem3.textContent = `${event.target.year.value}`;
    newListItem4.textContent = `${event.target.genre.value}`;
    newListItem5.textContent = `${event.target.legendary.value}`;
    const list = document.querySelector('#movie-list');
    newListItem.appendChild(newListItem1);
    newListItem.appendChild(newListItem2);
    newListItem.appendChild(newListItem3);
    newListItem.appendChild(newListItem4);
    newListItem.appendChild(newListItem5);
